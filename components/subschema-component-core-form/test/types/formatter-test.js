...
describe('types/formatter', function () {
    describe('1 (###) ###-####', function(){
        ...
        it('should format correctly as typed', function(){
            ...
            result = pattern('23')
            expect(result.value).to.eql('1 (23');
            result = pattern('234')
            expect(result.value).to.eql('1 (234) ');
            result = pattern('2345')
            expect(result.value).to.eql('1 (234) 5');
            result = pattern('23456')
            expect(result.value).to.eql('1 (234) 56');
            result = pattern('234567')
            expect(result.value).to.eql('1 (234) 567-');
            result = pattern('2345678')
            expect(result.value).to.eql('1 (234) 567-8');
            result = pattern('23456789')
            expect(result.value).to.eql('1 (234) 567-89');
            result = pattern('234567890')
            expect(result.value).to.eql('1 (234) 567-890');
            result = pattern('2345678901')
            expect(result.value).to.eql('1 (234) 567-8901');
    ...
    describe('###-##-#### ', function () {
        ...
        it('should format  an invalid delimiter', function () {
            ...
            expect(result.value).to.eql('123-44-5678');

            result = pattern('123456789');
            expect(result.isValid).to.eql(true);
            expect(result.value).to.eql('123-45-6789');

        });
        it('should format incomplete 3rd group', function () {
            result = pattern('12345');
            expect(result.value).to.eql('123-45-');
    ...
    describe('00000', function () {
        ...
        it('should pad 10', function () {
            result = pattern('10');
            expect(result.value).to.eql('00010');
        });

        it('should pad 1000', function () {
            result = pattern('1000');
            expect(result.value).to.eql('01000');
        });
    });
